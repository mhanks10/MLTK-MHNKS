...
╚══██╔══╝██║██║ ██╔╝     ████╗  ██║██╔════╝
   ██║   ██║█████╔╝█████╗██╔██╗ ██║█████  
   MMMMM NNNNNNNN HHHHHHH KKKKKK SSSSS
   ██║   ██║██╔═██╗╚════╝██║╚██╗██║██╔══╝  
   ██║   ██║██║  ██╗     ██║ ╚████║██║     
   ╚═╝   ╚═╝╚═╝  ╚═╝     ╚═╝  ╚═══╝╚═╝                                                              
          By @@welldone123 - @mhnks_p1
""")
user=input(f'[{Fore.BLACK}?{Fore.WHITE}] Enter The user : ');print('\n')
...
if 'undefined' in re.findall('<title>(.*?)</title>',rdu.text)[0]:exit(f'[!] user Not Found ! , {user}')
    ...
    print(f'[{Fore.BLACK}+{Fore.WHITE}] user id : ',re.findall('"userId":"(.*?)",',rdu.text)[0])
    print(f'[{Fore.RED}+{Fore.WHITE}] verified : ',re.findall('"verified":(.*?),',rdu.text)[0])
    print(f'[{Fore.BLACK}+{Fore.WHITE}] username : ',re.findall('"username":"(.*?)",',rdu.text)[0])
    print(f'[{Fore.RED}+{Fore.WHITE}] Likes : ',re.findall('"likeCount":(.*?),',rdu.text)[0])    
    print(f'[{Fore.BLACK}+{Fore.WHITE}] Followers : ',re.findall('"followerCount":(.*?),',rdu.text)[0])
    print(f'[{Fore.RED}+{Fore.WHITE}] Following : ',re.findall('"followingCount":(.*?),',rdu.text)[0])
    print(f'[{Fore.BLACK}+{Fore.WHITE}] Video Count : ',re.findall('"videoCount":(.*?)},',rdu.text)[0])
else:exit('[!] Error !! ')
